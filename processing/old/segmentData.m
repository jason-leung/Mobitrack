function [segment_inds] = segmentData(t, roll, pitch, createPlots)
...
if createPlots
 ...
 if(createPlots > 1)
     ...
     % Plot ZVC
     figure, hold on,
     plot(t(2:end), pitch(2:end)), plot(t, ZVC .*epsilon + mean(pitch), 'Color', 'g')
     
 end
    figure, hold on
    plot(pitch), plot(ZVC .*epsilon + mean(pitch), 'Color', 'g')
